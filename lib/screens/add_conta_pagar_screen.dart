...
class AddContaPagarScreen extends StatefulWidget {
  final ContaPagar? contaPagar;

  const AddContaPagarScreen({super.key, this.contaPagar});

  @override
  State<AddContaPagarScreen> createState() => _AddContaPagarScreenState();
}

class _AddContaPagarScreenState extends State<AddContaPagarScreen> {
  ...
  void initState() {
    super.initState();
    _mes = widget.contaPagar?.mes ?? DateTime.now().month;
    _ano = widget.contaPagar?.ano ?? DateTime.now().year;
    _data = widget.contaPagar?.data;
    _grupoId = widget.contaPagar?.grupoId;
    _pago = widget.contaPagar?.pago ?? false;
    _recorrente = widget.contaPagar?.recorrente ?? false;
    _isDigital = widget.contaPagar?.isDigital ?? true;
    _mesFim = widget.contaPagar?.mesFim ?? _mes;
    _anoFim = widget.contaPagar?.anoFim ?? _ano;
    if (widget.contaPagar != null) {
      _descricaoController.text = widget.contaPagar!.descricao;
      _valorController.text = widget.contaPagar!.valor.toStringAsFixed(2);
    }
    _carregarGrupos();
  }

  void _carregarGrupos() {
    _grupos = _storage.grupos.where((gr) => !gr.isReceita && !gr.isRecebivel).toList();
  }
  ...
  Future<void> _salvar() async {
    ...
    final conta = ContaPagar(
      id: widget.contaPagar?.id ?? DateTime.now().millisecondsSinceEpoch.toString(),
    ...
    String? erro;
    if (widget.contaPagar != null) {
      erro = await _storage.atualizarContaPagar(conta);
    } else {
      erro = await _storage.adicionarContaPagar(conta);
    }
    if (erro != null) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    final editing = widget.contaPagar != null;

    return Scaffold(
      appBar: AppBar(
        title: Text(editing ? 'Editar Conta a Pagar' : 'Nova Conta a Pagar'),
        backgroundColor: Colors.red,
